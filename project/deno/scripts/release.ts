...
// deno compile は Go の ldflags のような version 注入ができないため、
// entrypoint に `export const VERSION = '<name> <semver>'` を宣言して手動 bump する
const VERSION_FILE = 'src/main.ts'
// project 名は directory 名由来で `.` 等を含みうるため、env var 名として安全な文字に丸める
const RELEASE_VERSION_ENV =
  PROJECT_NAME.toUpperCase().replace(/[^A-Z0-9]/g, '_') +
  '_RELEASE_VERSION'
...
function parseArgs(): {
  ...
  const version = rawVersion ?? Deno.env.get(RELEASE_VERSION_ENV)
...
function versionPattern(): RegExp {
  // project 名に `.` 等の regex metacharacter が含まれても誤 match しないよう escape する
  const name = PROJECT_NAME.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  return new RegExp(`export const VERSION = '${name} (\\d+\\.\\d+\\.\\d+)'`)
}

async function bumpVersion(version: string): Promise<void> {
  const source = await Deno.readTextFile(VERSION_FILE)
  const currentVersion = source.match(versionPattern())?.[1]

  if (currentVersion === undefined) {
    throw new Error(
      `VERSION declaration was not found in ${VERSION_FILE} or already uses a non-standard format.`,
    )
  }
  if (currentVersion === version) {
    console.log(
      `VERSION is already ${PROJECT_NAME} ${version}; leaving ${VERSION_FILE} unchanged.`,
    )
    return
  }
  ...
  const next = source.replace(
    versionPattern(),
    `export const VERSION = '${PROJECT_NAME} ${version}'`,
  )

  await Deno.writeTextFile(VERSION_FILE, next)
}
...
async function assertCliVersion(version: string): Promise<void> {
  ...
  const expected = `${PROJECT_NAME} ${version}`
